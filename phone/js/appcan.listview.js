...
var model_thickLine = '<li <%if(data.id){%>id="<%=data.id%>"<%}%> style="background-color: #fff;" class="ub bc-border t-bla ub-ac lis umar-b umar-l umar-r"  data-index="<%=index%>">\
<%if(option.hasCheckbox && option.align=="left"){%>\
<div class="checkbox umar-r"><input type="checkbox" class="uabs ub-con" ></div>\
<%}%>\
<%if(option.hasRadiobox && option.align=="left"){%>\
...
<li class="bc-text ub ub-ver ut-m umar-b line<%=option.multiLine%>"><%=data.title%></li>\
<%if(data.describe){%><li class="line2 umar-b ub-ver ub ut-m ulev-1 sc-text "  ><%=data.describe%> </li><%}%>\
<%if(data.note){%><li class="ulev-2 sc-text-hint "><%=data.note%></li><%}%>\
</ul>\
<%if(option.hasSubTitle){%>\
<ul class="ub ub-ver ub-ae umar-r">\
<%if(data.subTitle){%><li class="bc-text umar-b lv_subTitle"><%=data.subTitle%></li><%}%>\
<%if(data.subDescribe){%><li class="line2 umar-b ub-ver ub ut-m ulev-1 sc-text " ><%=data.subDescribe%></li><%}%>\
<%if(data.subNote){%><li class="ulev-2 sc-text-hint "><%=data.subNote%></li><%}%>\
</ul>\
<%}%>\
<%if(option.hasControl && data.switchBtn){%>\
<div class="switch uba bc-border <%if(data.switchBtn.mini){%>switch-mini<%}%>" data-checked="<%=data.switchBtn.value%>">\
