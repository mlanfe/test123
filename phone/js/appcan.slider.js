appcan.define("slider", function($, exports, module) {
    ...
    SliderView.prototype = {
            ...
            if(self.option.hasCircle){
                 ...
                 if(index > self.option.itemCount){
                    ...
                    for(var i = 0;i < name.length;i++){
                        if(i == index){
                            name[i].style.cssText= 'margin-right : .5em;float: left;padding: 0.25em;background-color: #0082cf;border-radius: 50%;';
                        }else{
        ...
        set:function(data){
            ...
            var width = self.ele.offset().width / self.option.itemCount;
            if(self.option.hasLabel){
                self.label = self.label || $('<div class="uinn8 ulev-1 ut-s label22 sc-text-hint"></div>');
                self.ele.append(self.label);
            }
            if(self.option.hasCircle){
                self.Circle = $('<div class="label1" style="background-color:#fff;height:1.5em;width: 100%;position: absolute;left: 0px;bottom: 0px;"></div>');
                ...
                var zamcon = $('<div class="labelcon1" name="labContent" style="margin-right : .5em;float: left;padding: 0.25em;background-color: #0082cf;border-radius: 50%;"></div>');
